...
(
	name		= "NAND Gate",
	description	= "A simple NAND gate."
)
public final class NAND extends Gate
{
	public NAND(ComponentCollection parent,String name)
	{
		super(parent,name);
	...
	private static final class Diagram extends ComponentDiagram implements Eldritch
	{
		Diagram(boolean p)
		{
			...
			path.moveTo(0, 0);
			path.lineTo(40, 0);
			path.quadTo(80, 0, 80, 40);
			path.quadTo(90, 20, 100, 40);
			path.quadTo(90, 60, 80, 40);
			path.quadTo(80, 80, 40, 80);
			path.lineTo(0, 80);
			path.closePath();
			setPath(path);
			if(p)
			{
				this.addPin(new Pin(new Point(10, 40), new Point(0, 40), "in"));
	...
	private NAND(DeserializingStream in) throws IOException, DeserializingException, InstantiationException
	{
		super(in);
	}
}
