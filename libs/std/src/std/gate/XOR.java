...
(
	name		= "XOR Gate",
	description	= "A simple XOR gate."
)
public final class XOR extends Gate
{
	public XOR(ComponentCollection parent,String name)
	{
		super(parent,name);
	...
	private static final class Diagram extends ComponentDiagram implements Eldritch
	{
		Diagram(boolean p)
		{
			...
			path.moveTo(0, 0);
			path.lineTo(5, 0);
			path.curveTo(25, 30, 25, 70, 5, 100);
			path.lineTo(0, 100);
			...
			path.lineTo(15, 100);
			path.curveTo(35, 70, 35, 30, 15, 0);
			path.closePath();
			setPath(path);
			if(p)
			{
				this.addPin(new Pin(new Point(35, 50), new Point(0, 50), "in"));
	...
	private XOR(DeserializingStream in) throws IOException, DeserializingException, InstantiationException
	{
		super(in);
	}
}
