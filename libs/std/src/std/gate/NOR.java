...
(
	name		= "NOR Gate",
	description	= "A simple NOR gate."
)
public final class NOR extends Gate
{
	public NOR(ComponentCollection parent,String name)
	{
		super(parent,name);
	...
	private static final class Diagram extends ComponentDiagram implements Eldritch
	{
		Diagram(boolean p)
		{
			...
			if(p)
			{
				this.addPin(new Pin(new Point(20, 40), new Point(0, 40), "in"));
	...
	private NOR(DeserializingStream in) throws IOException, DeserializingException, InstantiationException
	{
		super(in);
	}
}
