...
public class GatedDFlipflop implements Testable
{
	@Override public final void test(TestSuite testsuite)
	{
		for(int i = 1; i < 15; ++i)
		{
			...
			Setable flipflop = testsuite.addComponent(std.flipflop.GatedDFlipFlop.class).setAll(clk, e, d, q, nq);
			...
			for(int j = 0; j < (1 << i); ++j)
			{
				...
				for(int m = 0; m < i; ++m)
					if(oclk[m] == L && clk.compareBit(m, H) && e.compareBit(m, H))
						old[m] = d.getBit(m);
